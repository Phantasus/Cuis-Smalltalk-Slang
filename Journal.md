...
It has no generation timestamp, which would normally mark it as
generated by Slang. The first commit of it in the opensmalltalk-vm
repository seems to be on `Jun 26, 2010` by Eliot Miranda. The
very first Squeak (1.3) doesn't have it. This is obvious as this
version only supports internal primitives and not external named
primitives. What was then the first version of named primitives?

It's interesting to note too, that the notion of the interpreter
proxy to be "setted" by the `setInterpreter` plugin call, which
does set the methods used in the plugin normally, comes from the
early day usage of globally calling the vm primitives directly.
This  probably was the easiest way of enabling legacy code to still
do as it did but providing everything as just function pointers
and then calling them in the plugin, so you could partially just
copy old code around and don't need to rewrite them.
So like this:

````C
  static sqInt *byteAt(sqInt oop);
  
  sqInt setInterpreter(vm_t *ptr) {
    byteAt = ptr->byteAt;
    interpreterProxy = ptr;
  }
  
  sqInt myPrimitive(void) {
    byteAt(interpreterProxy->pop());
  }
````

I exported the interpreter proxy in-image simulation class,
maybe I could generate the header file from it instead of using
a header file. The problem with header files is that the order
of the struct needs to be kept 1:1 as for C the order is really
important.
